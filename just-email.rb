...
require_relative 'send_onlyaccount_email.rb' #used to send user account and password emails
...
parser = OptionParser. new do|opts|
  ...
  opts.on('-e', '--environment environment', 'Enter Target Enviornment to send email for (ie: prod, backup, sandbox)') do |environment|
...
#Set Environment to send email to
if options[:environment] == nil
    puts ""
    print green("Enter Target Enviornment to send email for (ie: prod, backup, sandbox)")
...
#  else
    ...
    send_onlyaccount_email("#{options[:username]}", "#{env}", "#{params["#{env}"]['pcf_login_url']}", "#{params["#{env}"]['pcf_api_url']}", "#{params["#{env}"]['send_email_from']}", "#{params["#{env}"]['send_email_domain']}","#{params["#{env}"]['send_email_user']}","#{params["#{env}"]['send_email_api_key']}","#{params["#{env}"]['smtp_endpoint']}")
    puts "********************************************************************************************************"
